    ...
    public partial class frm_MainForm_new : Form
    {
        ...
        public frm_MainForm_new()
        {
            try
            {
                ...
                v.MdiTabCreated += PanelManager_MdiTabCreated;
                KeyDown += frm_MainForm_new_KeyDown;
                v.MdiTabClosed += PanelManager_MdiTabClosed;
                v.MdiTabDoubleClick += PanelManager_MdiTabDoubleClick;
                v.MdiTabClick += PanelManager_MdiTabClick;
             ...
             //   lblCopyright.Click += lblCopyright_Click;
                ...
                {
                    Utility.ShowMsg("Lỗi khởi tạo Webservice.Url"+ exWs.Message);
                }
                if (PropertyLib._ConfigProperties.RunUnderWS)
                {
                    ...
                    string UID = "";
                    string PWD = "";
                    WS._AdminWS.GetConnectionString(ref DataBaseServer, ref DataBaseName, ref UID, ref PWD, GetIpAddress());
                    //Utility.ShowMsg(WS._AdminWS.Url.ToString());
                    //Utility.ShowMsg(string.Format("{0}{1}{2}{3}", DataBaseServer, DataBaseName, UID, PWD));
        ...
        private void CheckVersion()
        {
            try
            {
                ...
                {
                    log.Trace(dtData.Rows.Count.ToString());
                    string fullfilePath = "";
        ...
        private void SetsyncDateTime()
        {
            ...
            Utility.FormatSystemDateTime("dd/MM/yyyy");
            Utility.FormatSystemSymbol("");
        }
        ...
        private void AutoOpen(bool forced2Open)
        {
            try
            {
                ...
                if (PropertyLib._AppProperties.AutoOpen || forced2Open)
                {
                    ...
                    UIMdiChildTab _item = v.FindMdiTab(dic[_id.ToString()].Form);
        ...
        private void LoadMenuStyleDocking()
        {
            try
            {
                ...
                {
                    v.DefaultPanelSettings.CloseButtonVisible = false;
            ...
            {
                pMain.ResumeLayout();
                v.VisualStyle = PropertyLib._AppProperties._PanelVisualStyle;
        ...
        private TreeNode AddNewTreeNode(TreeView objTreeView, TreeNode objParentTreeNode, DataRow drItems)
        {
            ...
            objnewChildNode.Tag = sFunctionName + "#" + Utility.sDbnull(drItems["isTabView"], "0") + "#" + sThamSo + "#" + IRoleID;
                        ...
                        if (string.IsNullOrEmpty(sThamSo))
                        {
                            ...
                            if (form != null)
                            {
                                ...
                                }
                                {
                                    if (dic.ContainsKey(_ID))
                                    {
                                        UIMdiChildTab _item = v.FindMdiTab(dic[_ID].Form);
                ...
                if (File.Exists(DllName))
                {
                    ...
                    foreach (Type objType in type)
                    {
                        if (objType.Name.ToUpper() == sFormName.ToUpper())
                        {
                            ...
                            {
                                log.Trace(sThamSo.ToString());
                                log.Trace(arrFunctionName.ToString());
                                object obj = new DynamicInitializer().CreateInstance(objType, new object[] {sThamSo});
                                var form = obj as Form;
                                if (form != null)
                                {
